...
{
	local fwRETURN=""
	#echo "in _fwVariable, _fwVariableARRAY= " ${_fwVariableARRAY[@]} > /dev/stderr
	for (( fwI = 0; fwI < ${#_fwVariableARRAY[@]} ; ++fwI ))
	do
		local fwTEMP=(${_fwVariableARRAY[${fwI}]})
		#echo "in _fwVariable, fwTEMP[0]= " ${fwTEMP[0]} > /dev/stderr
		#echo "in _fwVariable, fwTEMP[1]= " ${fwTEMP[1]} > /dev/stderr
		#echo "in _fwVariable, 1= " ${1} > /dev/stderr
		#read
...
function _fwOptionList()
{
	...
	then
		echo "-P ${_fwMAXPROC}"
		#echo "in _fwOptionList,_fwMAXPROC -P ${_fwMAXPROC}" > /dev/stderr
	fi
	for (( fwI = 0; fwI < ${#_fwOptionARRAY[@]} ; ++fwI ))
	do
		echo "-p ${_fwOptionARRAY[${fwI}]}"
		#echo "in _fwOptionList -p ${_fwOptionARRAY[${fwI}]}" > /dev/stderr
	done
}
...
function FwOptions()
{
	...
	do
		...
		#echo "in FwOptions, _fwOptionARRAY: " ${_fwOptionARRAY[${fwI}]}
		(( ++fwI ))
		shift
	done
	...
	#echo "In FwOptions, fwMessageOPTIONS= " ${fwMessageOPTIONS}
	#echo "In FwOptions, _fwGDSDomain= " ${_fwGDSDomain}
	#echo "In FwOptions, _fwOptionList= \n" 
	#echo $(_fwOptionList)	
	local fwLINES=($(${_fwModelBIN} ${_fwGDSDomain} $(_fwOptionList) -T ${fwMessageOPTIONS}))
	#echo "In FwOptions, fwLINES1: " ${fwLINES[@]}
	local fwLINES=($(${_fwModelBIN} ${_fwGDSDomain} $(_fwOptionList) -T ${fwMessageOPTIONS} | grep "XXXX"  | cut -c15-45,58-64,77-80,81-85,86-94))
	#echo "In FwOptions, fwLINES2: " ${fwLINES[@]}
	#echo "In FwOptions, #fwLINES: " ${#fwLINES[@]}
	for (( fwVARnum = 0; fwVARnum < ${#fwLINES[@]} / 5 ; ++fwVARnum ))
	do
		_fwVariableITEM="${fwLINES[(( fwVARnum * 5    ))]}"
		#echo "In FwOptions, _fwVariableITEM= " ${_fwVariableITEM}

		local fwVARTYPE="${fwLINES[(( fwVARnum * 5 + 1))]}"
		#echo "In FwOptions, fwVARTYPE= " ${fwVARTYPE}

		local     fwSET="${fwLINES[(( fwVARnum * 5 + 2))]}"
		#echo "In FwOptions, fwSET= " ${fwSET}

		local    fwFLUX="${fwLINES[(( fwVARnum * 5 + 3))]}"
		#echo "In FwOptions, fwFLUX= " ${fwFLUX}

		local fwINITIAL="${fwLINES[(( fwVARnum * 5 + 4))]}"
		#echo "In FwOptions, fwINITIAL= " ${fwINITIAL}
		...
		#echo "In FwOptions, _fwVariableARRAY: " ${_fwVariableARRAY[${fwVARnum}]}		
		if [ "${fwSET}"     == "no"  ]
		then
...
function FwOutputs()
{
	...
	while [ "${1}"	!= "" ]
	do
		#echo "in FwOutputs, {1} :" ${1}
		if [ "$(_fwVariable ${1})" == "" ]
		#echo "in FwOutputs, _fwVariable {0} :" $(_fwVariable ${0})
		#echo "in FwOutputs, _fwVariable {1} :" $(_fwVariable ${1})
		then
			[ "${FwWARNINGS}" == "on" ] && echo "Skipping output variable: ${1}."
		else
			_fwOutputARRAY[${fwI}]="${1}"
		#	echo "in FwOutputs " ${_fwOutputARRAY[${fwI}]}
			(( ++fwI ))
		fi
		shift
	done
...
function _fwPreprocess()
{
	...
	do
		...
		then
			...
			[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )) ; }
			${_fwRGISBIN}rgis2ds -m "${_fwRGISDomainFILE}" "${fwSOURCE[4]}" "${fwFILENAME}" &
#echo "${_fwRGISDomainFILE}" "${fwSOURCE[4]}" "${fwFILENAME}" 			
			(( ++fwPROC ))
		fi
	done
	wait
...
function _fwPostprocess()
{
   local fwVERSION="${1}"
	...
	do
		echo ${fwI}
		local fwVARIABLE1="${_fwOutputARRAY[${fwI}]}"
		echo ${fwVARIABLE1}
		local    fwAMODE1="$(_fwVariable "${fwVARIABLE1}")"
		[ "${fwAMODE1}" == "" ] && { echo "Skipping undefinded variable [${fwVARIABLE1}]"; continue; }
		local fwGDSFileNAME1="$(FwGDSFilename "${fwVARIABLE1}" "Output" "${fwVERSION}" "${fwYEAR}" "d")"
		[ -e "${fwGDSFileNAME1}" ] || local fwGDSFileNAME1="$(FwGDSFilename "${fwVARIABLE1}" "State" "${fwVERSION}" "${fwYEAR}" "d")"
		[ -e "${fwGDSFileNAME1}" ] || { echo "Skipping missing variable [${fwVARIABLE1}]"; echo ${fwGDSFileNAME1}; continue; }
		[ -e "${_fwRGISResultsDIR}/${_fwDomainNAME}/${fwVARIABLE1}" ] || mkdir -p "${_fwRGISResultsDIR}/${_fwDomainNAME}/${fwVARIABLE1}"
		(( ++fwI ))

		local fwVARIABLE2="${_fwOutputARRAY[${fwI}]}"
		echo ${fwVARIABLE2}
		local    fwAMODE2="$(_fwVariable "${fwVARIABLE2}")"
		[ "${fwAMODE2}" == "" ] && { echo "Skipping undefinded variable [${fwVARIABLE2}]";  }
		local fwGDSFileNAME2="$(FwGDSFilename "${fwVARIABLE2}" "Output" "${fwVERSION}" "${fwYEAR}" "d")"
		[ -e "${fwGDSFileNAME2}" ] || local fwGDSFileNAME2="$(FwGDSFilename "${fwVARIABLE2}" "State" "${fwVERSION}" "${fwYEAR}" "d")"
		[ -e "${fwGDSFileNAME2}" ] || { echo "Skipping missing variable [${fwVARIABLE2}]"; echo ${fwGDSFileNAME2};  }
		[ -e "${_fwRGISResultsDIR}/${_fwDomainNAME}/${fwVARIABLE2}" ] || mkdir -p "${_fwRGISResultsDIR}/${_fwDomainNAME}/${fwVARIABLE2}"
		(( ++fwI ))
		
		local fwVARIABLE3="${_fwOutputARRAY[${fwI}]}"
		echo ${fwVARIABLE3}
		local    fwAMODE3="$(_fwVariable "${fwVARIABLE3}")"
		[ "${fwAMODE3}" == "" ] && { echo "Skipping undefinded variable [${fwVARIABLE3}]";  }
		local fwGDSFileNAME3="$(FwGDSFilename "${fwVARIABLE3}" "Output" "${fwVERSION}" "${fwYEAR}" "d")"
		[ -e "${fwGDSFileNAME3}" ] || local fwGDSFileNAME3="$(FwGDSFilename "${fwVARIABLE3}" "State" "${fwVERSION}" "${fwYEAR}" "d")"
		[ -e "${fwGDSFileNAME3}" ] || { echo "Skipping missing variable [${fwVARIABLE3}]"; echo ${fwGDSFileNAME3};  }
		[ -e "${_fwRGISResultsDIR}/${_fwDomainNAME}/${fwVARIABLE3}" ] || mkdir -p "${_fwRGISResultsDIR}/${_fwDomainNAME}/${fwVARIABLE3}"
		(( ++fwI ))
		
		local fwVARIABLE4="${_fwOutputARRAY[${fwI}]}"
		echo ${fwVARIABLE4}
		local    fwAMODE4="$(_fwVariable "${fwVARIABLE4}")"
		[ "${fwAMODE4}" == "" ] && { echo "Skipping undefinded variable [${fwVARIABLE4}]";  }
		local fwGDSFileNAME4="$(FwGDSFilename "${fwVARIABLE4}" "Output" "${fwVERSION}" "${fwYEAR}" "d")"
		[ -e "${fwGDSFileNAME4}" ] || local fwGDSFileNAME4="$(FwGDSFilename "${fwVARIABLE4}" "State" "${fwVERSION}" "${fwYEAR}" "d")"
		[ -e "${fwGDSFileNAME4}" ] || { echo "Skipping missing variable [${fwVARIABLE4}]"; echo ${fwGDSFileNAME4};  }
		[ -e "${_fwRGISResultsDIR}/${_fwDomainNAME}/${fwVARIABLE4}" ] || mkdir -p "${_fwRGISResultsDIR}/${_fwDomainNAME}/${fwVARIABLE4}"
		
		echo "starting days"
		if [ "${_fwDAILYOUTPUT}" == "on" ]
		then
			local fwRGISFileNAME1="$(FwRGISFilename "${fwVARIABLE1}" "${fwVERSION}" "d" "${fwYEAR}")"
			local fwRGISFileNAME2="$(FwRGISFilename "${fwVARIABLE2}" "${fwVERSION}" "d" "${fwYEAR}")"
			local fwRGISFileNAME3="$(FwRGISFilename "${fwVARIABLE3}" "${fwVERSION}" "d" "${fwYEAR}")"
			local fwRGISFileNAME4="$(FwRGISFilename "${fwVARIABLE4}" "${fwVERSION}" "d" "${fwYEAR}")"
			[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
			${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE1} ${fwVERSION} (${FwDomainRES}, Daily${fwSUFFIX})"\
			                     -m ${_fwRGISDomainFILE} -d "${_fwDomainNAME}" -u "${fwVARIABLE1}" -s blue ${fwGDSFileNAME1} ${fwRGISFileNAME1} &
			${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE2} ${fwVERSION} (${FwDomainRES}, Daily${fwSUFFIX})"\
			                     -m ${_fwRGISDomainFILE} -d "${_fwDomainNAME}" -u "${fwVARIABLE2}" -s blue ${fwGDSFileNAME2} ${fwRGISFileNAME2} &	
			${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE3} ${fwVERSION} (${FwDomainRES}, Daily${fwSUFFIX})"\
			                     -m ${_fwRGISDomainFILE} -d "${_fwDomainNAME}" -u "${fwVARIABLE3}" -s blue ${fwGDSFileNAME3} ${fwRGISFileNAME3} &	
			${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE4} ${fwVERSION} (${FwDomainRES}, Daily${fwSUFFIX})"\
			                     -m ${_fwRGISDomainFILE} -d "${_fwDomainNAME}" -u "${fwVARIABLE4}" -s blue ${fwGDSFileNAME4} ${fwRGISFileNAME4} &	    			                     
			(( ++fwPROC ))
		fi
		
		echo "starting months"
		[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
		local fwRGISFileNAME="$(FwRGISFilename "${fwVARIABLE1}" "${fwVERSION}" "m" "${fwYEAR}")"
		[ "${fwAMODE1}" == "" ] && { echo "Skipping undefinded variable [${fwVARIABLE1}]"; continue; }
		${_fwRGISBIN}dsAggregate -e month -a ${fwAMODE1} ${fwGDSFileNAME1} |\
		${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE1} ${fwVERSION} (${FwDomainRES}, Monthly${fwSUFFIX})"\
		                     -m ${_fwRGISDomainFILE}  -d "${_fwDomainNAME}" -u "${fwVARIABLE1}" -s blue - ${fwRGISFileNAME} &
		(( ++fwPROC ))
		[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
		local fwRGISFileNAME="$(FwRGISFilename "${fwVARIABLE1}" "${fwVERSION}" "a" "${fwYEAR}")"
		${_fwRGISBIN}dsAggregate -e year -a ${fwAMODE1} ${fwGDSFileNAME1} |\
		${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE1} ${fwVERSION} (${FwDomainRES}, Yearly${fwSUFFIX})"\
		                     -m ${_fwRGISDomainFILE} -d "${_fwDomainNAME}" -u "${fwVARIABLE1}"  -s blue - ${fwRGISFileNAME} &
		(( ++fwPROC ))
		
		[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
		local fwRGISFileNAME="$(FwRGISFilename "${fwVARIABLE2}" "${fwVERSION}" "m" "${fwYEAR}")"
		[ "${fwAMODE2}" == "" ] && { echo "Skipping undefinded variable [${fwVARIABLE1}]"; continue; }
		${_fwRGISBIN}dsAggregate -e month -a ${fwAMODE2} ${fwGDSFileNAME2} |\
		${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE2} ${fwVERSION} (${FwDomainRES}, Monthly${fwSUFFIX})"\
		                     -m ${_fwRGISDomainFILE}  -d "${_fwDomainNAME}" -u "${fwVARIABLE2}" -s blue - ${fwRGISFileNAME} &
		(( ++fwPROC ))
		[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
		local fwRGISFileNAME="$(FwRGISFilename "${fwVARIABLE2}" "${fwVERSION}" "a" "${fwYEAR}")"
		${_fwRGISBIN}dsAggregate -e year -a ${fwAMODE2} ${fwGDSFileNAME2} |\
		${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE2} ${fwVERSION} (${FwDomainRES}, Yearly${fwSUFFIX})"\
		                     -m ${_fwRGISDomainFILE} -d "${_fwDomainNAME}" -u "${fwVARIABLE2}"  -s blue - ${fwRGISFileNAME} &
		(( ++fwPROC ))                     
		                     
		[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
		local fwRGISFileNAME="$(FwRGISFilename "${fwVARIABLE3}" "${fwVERSION}" "m" "${fwYEAR}")"
		[ "${fwAMODE3}" == "" ] && { echo "Skipping undefinded variable [${fwVARIABLE1}]"; continue; }
		${_fwRGISBIN}dsAggregate -e month -a ${fwAMODE3} ${fwGDSFileNAME3} |\
		${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE3} ${fwVERSION} (${FwDomainRES}, Monthly${fwSUFFIX})"\
		                     -m ${_fwRGISDomainFILE}  -d "${_fwDomainNAME}" -u "${fwVARIABLE3}" -s blue - ${fwRGISFileNAME} &
		(( ++fwPROC ))
		[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
		local fwRGISFileNAME="$(FwRGISFilename "${fwVARIABLE3}" "${fwVERSION}" "a" "${fwYEAR}")"
		${_fwRGISBIN}dsAggregate -e year -a ${fwAMODE3} ${fwGDSFileNAME3} |\
		${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE3} ${fwVERSION} (${FwDomainRES}, Yearly${fwSUFFIX})"\
		                     -m ${_fwRGISDomainFILE} -d "${_fwDomainNAME}" -u "${fwVARIABLE3}"  -s blue - ${fwRGISFileNAME} &
		(( ++fwPROC ))
		
		[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
		local fwRGISFileNAME="$(FwRGISFilename "${fwVARIABLE4}" "${fwVERSION}" "m" "${fwYEAR}")"
		[ "${fwAMODE4}" == "" ] && { echo "Skipping undefinded variable [${fwVARIABLE1}]"; continue; }
		${_fwRGISBIN}dsAggregate -e month -a ${fwAMODE4} ${fwGDSFileNAME4} |\
		${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE4} ${fwVERSION} (${FwDomainRES}, Monthly${fwSUFFIX})"\
		                     -m ${_fwRGISDomainFILE}  -d "${_fwDomainNAME}" -u "${fwVARIABLE4}" -s blue - ${fwRGISFileNAME} &
		(( ++fwPROC ))
		[ "${fwPROC}" -ge "${_fwMAXPROC}" ] && { wait; (( fwPROC = 0 )); }
		local fwRGISFileNAME="$(FwRGISFilename "${fwVARIABLE4}" "${fwVERSION}" "a" "${fwYEAR}")"
		${_fwRGISBIN}dsAggregate -e year -a ${fwAMODE4} ${fwGDSFileNAME4} |\
		${_fwRGISBIN}ds2rgis -t "${_fwDomainNAME}, ${fwVARIABLE4} ${fwVERSION} (${FwDomainRES}, Yearly${fwSUFFIX})"\
		                     -m ${_fwRGISDomainFILE} -d "${_fwDomainNAME}" -u "${fwVARIABLE4}"  -s blue - ${fwRGISFileNAME} &
		(( ++fwPROC ))
...
function _fwSpinup()
{
	...
	echo "start of _fwSpinup" > /dev/stderr
	...
	do
		...
		echo "after echo list" > /dev/stderr
		...
		echo "after first do" > /dev/stderr
		...
		echo "after if (( fwPASS == 1 )) " > /dev/stderr
		...
		do
			...
			#echo ${fwOutputITEM[0]} > /dev/stderr
		...
		echo ${fwOptions} > "fwOptionsSpinUp.txt"
		[ "${FwVERBOSE}" == "on" ] && echo "   Passnum [${fwPASS}] started:  $(date '+%Y-%m-%d %H:%M:%S')"
		
		echo "before xargs ${_fwModelBIN} " > /dev/stderr
		
		if echo ${fwOptions} | xargs ${_fwModelBIN}
		then
			echo "after xargs ${_fwModelBIN} " > /dev/stderr
...
function _fwRun()
{
   local     fwVERSION="${1}"
	...
	do
		echo "Running" ${fwYEAR} >> /dev/stderr
		...
		if echo ${fwOptions} | xargs ${_fwModelBIN}
		then
			#echo ${fwOptions} > "fwOptionsRun${fwYEAR}.txt"			
