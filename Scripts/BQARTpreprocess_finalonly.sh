...
PROJECTDIR="/Volumes/Data/BQARTwbm_Runs/BQART_WBM-GPCCfull-NCEP_dTS-06min-NAmerica"

SCRIPT="${0##*/}"; SCRIPT="${SCRIPT%.sh}";

GHAAS_DIR="/Volumes/Data/ghaas"

      USAGE="Usage: <Domain> <Resolution> [dist|dist+gbc|prist|prist+gbc]"
      MODEL="${PROJECTDIR}/Model/WBMplus/bin/wbmplus.bin"
RGISARCHIVE="/Volumes/Data/RGISarchive"
  RGISPILOT="/Volumes/Data/RGISarchive"
...
FwArguments -s on -f on -u on -n 10 -D off  $* || exit -1  
